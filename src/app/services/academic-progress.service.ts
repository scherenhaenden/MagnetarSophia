...
export class AcademicProgressService {
  ...
  public parseJson(json: string): ExamRecord[] { const parsed = JSON.parse(json); if (!Array.isArray(parsed)) throw new Error('The provided JSON must describe an array of records.'); return this.sortRecords(parsed.map((item: JsonExamRecord) => this.mapJsonRecord(item))); }
